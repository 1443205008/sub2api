...
type Group struct {
	...
	// 多时段叠加倍率规则；左闭右开，支持跨午夜，规则不得重叠
	RateTimeRules []domain.GroupRateTimeRule `json:"rate_time_rules,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldRateTimeRules, group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldCodexModelsManifestConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldRateTimeRules:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field rate_time_rules", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.RateTimeRules); err != nil {
					return fmt.Errorf("unmarshal field rate_time_rules: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("rate_time_rules=")
	builder.WriteString(fmt.Sprintf("%v", _m.RateTimeRules))
	builder.WriteString(", ")
