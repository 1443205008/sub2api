...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID int64) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price,
		        service_tier_standard_multiplier, service_tier_fast_multiplier, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, output_price = $4, cache_write_price = $5, cache_read_price = $6, image_output_price = $7, per_request_price = $8,
		     service_tier_standard_multiplier = $9, service_tier_fast_multiplier = $10, platform = $11, updated_at = NOW()
		 WHERE id = $12`,
		modelsJSON, billingMode, pricing.InputPrice, pricing.OutputPrice, pricing.CacheWritePrice, pricing.CacheReadPrice,
		pricing.ImageOutputPrice, pricing.PerRequestPrice, pricing.ServiceTierStandardMultiplier, pricing.ServiceTierFastMultiplier,
		pricing.Platform, pricing.ID,
	)
	if err != nil {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []int64, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.ImageOutputPrice, &p.PerRequestPrice, &p.ServiceTierStandardMultiplier,
			&p.ServiceTierFastMultiplier, &p.CreatedAt, &p.UpdatedAt,
		); err != nil {
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (
		     channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price,
		     service_tier_standard_multiplier, service_tier_fast_multiplier
		 )
