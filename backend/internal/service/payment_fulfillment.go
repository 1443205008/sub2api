...
func (s *PaymentService) applyAffiliateRebateForOrder(ctx context.Context, o *dbent.PaymentOrder) error {
	...
	slog.Info("affiliate rebate: start", "orderID", o.ID, "userID", o.UserID, "amount", o.Amount)

	tx, err := s.entClient.Tx(ctx)
	if err != nil {
		slog.Error("affiliate rebate: begin tx failed", "orderID", o.ID, "error", err)
	...
	if err != nil {
		slog.Error("affiliate rebate: claim audit failed", "orderID", o.ID, "error", err)
		s.writeAuditLog(ctx, o.ID, "AFFILIATE_REBATE_FAILED", "system", map[string]any{
			"error": err.Error(),
		})
	...
	if !claimed {
		slog.Info("affiliate rebate: already claimed or skipped", "orderID", o.ID)
		return nil
	}

	rebateAmount, err := s.affiliateService.AccrueInviteRebate(txCtx, o.UserID, o.Amount)
	if err != nil {
		slog.Error("affiliate rebate: accrue failed", "orderID", o.ID, "userID", o.UserID, "error", err)
		s.writeAuditLog(ctx, o.ID, "AFFILIATE_REBATE_FAILED", "system", map[string]any{
			"error": err.Error(),
		})
	...
	if rebateAmount <= 0 {
		slog.Info("affiliate rebate: skipped", "orderID", o.ID, "userID", o.UserID, "reason", "no inviter or zero rebate")
		if err := s.updateClaimedAffiliateRebateAudit(txCtx, tx.Client(), o.ID, "AFFILIATE_REBATE_SKIPPED", map[string]any{
			...
			slog.Error("affiliate rebate: update audit to skipped failed", "orderID", o.ID, "error", err)
			s.writeAuditLog(ctx, o.ID, "AFFILIATE_REBATE_FAILED", "system", map[string]any{
				"error": err.Error(),
			})
		...
		if err := tx.Commit(); err != nil {
			slog.Error("affiliate rebate: commit skipped tx failed", "orderID", o.ID, "error", err)
			s.writeAuditLog(ctx, o.ID, "AFFILIATE_REBATE_FAILED", "system", map[string]any{
	...
	if err := s.updateClaimedAffiliateRebateAudit(txCtx, tx.Client(), o.ID, "AFFILIATE_REBATE_APPLIED", map[string]any{
		...
		slog.Error("affiliate rebate: update audit to applied failed", "orderID", o.ID, "error", err)
		s.writeAuditLog(ctx, o.ID, "AFFILIATE_REBATE_FAILED", "system", map[string]any{
			"error": err.Error(),
		})
	...
	if err := tx.Commit(); err != nil {
		slog.Error("affiliate rebate: commit failed", "orderID", o.ID, "rebateAmount", rebateAmount, "error", err)
		s.writeAuditLog(ctx, o.ID, "AFFILIATE_REBATE_FAILED", "system", map[string]any{
			"error": fmt.Sprintf("commit affiliate rebate tx: %v", err),
		})
		return fmt.Errorf("commit affiliate rebate tx: %w", err)
	}
	slog.Info("affiliate rebate: applied", "orderID", o.ID, "userID", o.UserID, "rebateAmount", rebateAmount, "baseAmount", o.Amount)
	return nil
}
